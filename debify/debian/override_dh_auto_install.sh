...
install -d -m 755 $DEB_INSTALL_ROOT$_sysconfdir/rpm
sed -e 's/@APR_NAME@/$prefix_name/g' \
    -e 's/@APR_VER@/$aprver/g' \
    -e 's,@APR_DIR@,$prefix_dir,g' \
    -e 's/@NAMESPACE@/$ns_name_/g' \
    $SOURCE2 > $DEB_INSTALL_ROOT$_sysconfdir/rpm/macros.$pkgname
...
mv $DEB_INSTALL_ROOT$prefix_inc/apr-$aprver/apr.h \
   $DEB_INSTALL_ROOT$prefix_inc/apr-$aprver/apr-$_arch.h
